...
type (
	usersModel interface {
		...
		FindByUsernameAndPwd(ctx context.Context, username string, password string) (*Users, error)
	}
...
func (m *defaultUsersModel) FindByUsernameAndPwd(ctx context.Context, username string, password string) (*Users, error) {
	var resp Users
	query := fmt.Sprintf("select %s from %s where `username` = ? and `password` = ? limit 1", usersRows, m.table)
